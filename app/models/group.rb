class Group < ActiveRecord::Base
	...
	def self.search(params)
		self.open_groups.category_groups(params[:category]).same_age(params[:age_group]).same_gender(params[:gender_group]).degree_groups(1).near([params[:latitude], params[:longitude]], 0.5).non_former_groups(params[:group_ids])[0]
	end
	...
	def join_group_notifications(new_user)
		users.each do |user|
			self.delay.notifications.create(user: user, poster: new_user, notification_type: "Join") unless user == new_user
		end
